...
def parse_args():
    ...
    parser.add_argument('--num-epoch', default=150, type=int) # unused
    ...
    parser.add_argument('--swin-type', default='small', type=str) # ['tiny', 'small', 'base']

    parser.add_argument('--save-interval', default=10, type=int)
    parser.add_argument('--resume', default='None', type=str) # unused

    parser.add_argument('--save-no', default=10, type=int)
...
def main():
    args = parse_args()
    in_c = 1024 if args.swin_type in ['base', 'large'] else 768
    model_name = 'swin_{}_patch4_window{}_{}'.format(args.swin_type, args.img_size//32, args.img_size)
    BaseNet = MyNet(model_backbone(name=model_name, pretrained=True), model_head(down_scale=args.htmp_ratio, in_channels=in_c))
    ...
    config = {'OUTPUT_DIR': 'output', 'LOG_DIR': 'log', 'DATA_DIR': '', 'GPU': args.cuda, 'WORKERS': args.num_workers, 'PRINT_FREQ': args.print_freq,
              ...
              'TRAIN': {'LR_FACTOR': 0.1, 'LR_STEP': [40, 80], 'LR': 0.001, 'OPTIMIZER': 'adam', 'MOMENTUM': 0.9, 'WD': 0.0001, 'NESTEROV': False, 'GAMMA1': 0.99, 'GAMMA2': 0.0, 'BEGIN_EPOCH': 0, 'END_EPOCH': 140, 'RESUME': False, 'CHECKPOINT': '', 'BATCH_SIZE': args.batch_size, 'SHUFFLE': True},
    ...
    optimizer = torch.optim.Adam(BaseNet.parameters(),
    ...
    train_dataset = coco(config, config.DATASET.ROOT, config.DATASET.TRAIN_SET,
        True, transforms.Compose([transforms.ToTensor(), normalize]))
    valid_dataset = coco(config, config.DATASET.ROOT, config.DATASET.TEST_SET,
        False, transforms.Compose([transforms.ToTensor(), normalize,]))
